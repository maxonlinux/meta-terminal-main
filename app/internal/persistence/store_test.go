...
func TestApplyTradeAccumulatesOrderFilledOnce(t *testing.T) {
	...
	batch := []events.Event{
		...
		events.EncodeTrade(events.TradeEvent{
			...
			Instrument:       inst,
			MakerOrderQty:    fixed.NewI(10, 0),
			MakerFilledAfter: fixed.NewI(1, 0),
			TakerOrderQty:    fixed.NewI(10, 0),
			TakerFilledAfter: fixed.NewI(1, 0),
		}),
		events.EncodeTrade(events.TradeEvent{
			TradeID:          1002,
			MakerOrderID:     makerOrder.ID,
			...
			Instrument:       inst,
			MakerOrderQty:    fixed.NewI(10, 0),
			MakerFilledAfter: fixed.NewI(2, 0),
			TakerOrderQty:    fixed.NewI(10, 0),
			TakerFilledAfter: fixed.NewI(2, 0),
		}),
	}
