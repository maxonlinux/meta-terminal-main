...
func (e *Engine) applyTrade(book *orderbook.OrderBook, match types.Match, writer outbox.Writer) error {
	...
	if writer != nil {
		_ = writer.Record(events.EncodeTrade(events.TradeEvent{
			...
			MakerOrderPrice:  match.MakerOrder.Price,
			TakerOrderPrice:  match.TakerOrder.Price,
			MakerOrderQty:    match.MakerOrder.Quantity,
			MakerFilledAfter: match.MakerOrder.Filled,
			TakerOrderQty:    match.TakerOrder.Quantity,
			TakerFilledAfter: match.TakerOrder.Filled,
		}))
	}
	return nil
}
