...
import (
	...
	"github.com/anomalyco/meta-terminal-go/services/risk"
...
func main() {
	...
	cfg := loadConfig()
	Mode := flag.String("mode", "all", "Mode: all, gateway, oms, portfolio, marketdata, risk, clearing")
	flag.Parse()
...
func runRisk(cfg Config) {
	svc, _ := risk.New(risk.Config{
		NATSURL:      cfg.NATSURL,
		StreamPrefix: cfg.StreamPrefix,
	})
	ctx, cancel := context.WithCancel(context.Background())
