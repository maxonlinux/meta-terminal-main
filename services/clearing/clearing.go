...
func (s *Service) Start(ctx context.Context) error {
	s.nats.Subscribe(ctx, "clearing.trade", "clearing-trades", s.handleTrade)
	s.nats.Subscribe(ctx, "orders.>", "clearing-orders", s.handleOrder)
	s.nats.Subscribe(ctx, "clearing.reserve", "clearing-reserve", s.handleReserve)
	s.nats.Subscribe(ctx, "clearing.release", "clearing-release", s.handleRelease)
...
func (s *Service) handleRelease(data []byte) {
	...
	s.nats.PublishBytes(context.Background(), "portfolio.release", buf)
...
func (s *Service) handleTrade(data []byte) {
	...
	switch msgType {
	case 0x01:
		s.decodeAndProcessTrade(data, offset)
	default:
		log.Printf("clearing: unknown message type 0x%02x", msgType)
	}
}
...
func (s *Service) decodeAndProcessTrade(data []byte, offset int) {
	...
	symbolLen := int(data[offset])
	offset++
	if len(data) < offset+symbolLen+50 {
		return
	}
	...
	trade := &types.Trade{}
	trade.Symbol = symbol
	trade.TakerID = uint64(binary.LittleEndian.Uint64(data[offset:]))
	offset += 8
	trade.MakerID = uint64(binary.LittleEndian.Uint64(data[offset:]))
	offset += 8
	trade.TakerOrderID = types.OrderID(binary.LittleEndian.Uint64(data[offset:]))
	offset += 8
	trade.MakerOrderID = types.OrderID(binary.LittleEndian.Uint64(data[offset:]))
	offset += 8
	trade.Price = types.Price(binary.LittleEndian.Uint64(data[offset:]))
	offset += 8
	trade.Quantity = types.Quantity(binary.LittleEndian.Uint64(data[offset:]))
	offset += 8
	...
	trade.TakerLeverage = int8(data[offset])
	offset++
	trade.MakerLeverage = int8(data[offset])
...
func (s *Service) publishBalanceUpdate(userID uint64, asset string, amount int64, isAdd bool) {
...
func (s *Service) publishMarginUpdate(userID uint64, symbol string, margin int64, isAdd bool) {
	...
	s.nats.PublishBytes(context.Background(), "portfolio.margin", buf)
}

func (s *Service) publishPositionUpdate(userID uint64, symbol string, size int64, price int64, side int8, leverage int8) {
	...
	s.nats.PublishBytes(context.Background(), "positions."+symbol, buf)
}
