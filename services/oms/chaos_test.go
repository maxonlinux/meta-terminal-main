	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func(goroutineID int) {
			...
			for j := 0; j < numOrdersPerGoroutine; j++ {
				input := &OrderInput{
					UserID:   uint64((goroutineID+j)%50 + 1),
					Symbol:   "BTCUSDT",
	...
	for i := 0; i < numIterations; i++ {
		...
		go func(iterID int) {
			...
			if result != nil && result.OrderID != 0 {
				err := svc.CancelOrder(ctx, uint64(iterID%10+1), result.OrderID)
				if err != nil {
