...
func BenchmarkNATS_Publish(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		n.Publish(ctx, "test.subject", map[string]interface{}{
...
func BenchmarkOMS_ReduceOnlyCapping(b *testing.B) {
	reg := registry.New()
	svc, _ := New(Config{
		NATSURL:      "nats://localhost:4222",
		StreamPrefix: "bench",
...
func BenchmarkOMS_AdjustReduceOnlyOrders(b *testing.B) {
	...
	svc.mu.Lock()
	if svc.orders[1] == nil {
		svc.orders[1] = make(map[types.OrderID]*types.Order)
	}
	for i := 0; i < 100; i++ {
		order := pool.GetOrder()
	...
	for i := 0; i < b.N; i++ {
		svc.adjustReduceOnlyOrders(1, "BTCUSDT", 50)
	}
}
