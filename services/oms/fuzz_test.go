...
func FuzzOMS_ConditionalOrders(f *testing.F) {
	f.Fuzz(func(t *testing.T, seed int64, triggerPrice int64, side int8, orderType int8) {
		...
		input := &OrderInput{
			UserID:         uint64(rand.Intn(1000) + 1),
			Symbol:         "BTCUSDT",
...
func FuzzOMS_ReduceOnlyCapping(f *testing.F) {
	f.Fuzz(func(t *testing.T, seed int64, qty int64, posSize int64) {
		if qty <= 0 || posSize <= 0 {
			return
		}
		...
		svc.mu.Lock()
		if svc.positions[1] == nil {
			svc.positions[1] = make(map[string]PositionInfo)
		}
		svc.positions[1]["BTCUSDT"] = PositionInfo{Size: posSize, Side: constants.SIDE_LONG}
		svc.mu.Unlock()
		...
		if result != nil {
			if input.Qty > posSize && result.Status == constants.ORDER_STATUS_NEW {
				t.Fatalf("ReduceOnly order should be capped to position size")
			}
		}
