...
import (
	...
	"github.com/anomalyco/meta-terminal-go/services/oms/events"
)
...
type Service struct {
	...
	orders               map[uint64]map[types.OrderID]*types.Order
	triggerMonitor       *triggers.Monitor
	registry             *registry.Registry
	reduceOnlyCommitment map[uint64]map[string]int64
	mu                   sync.RWMutex
}
...
func New(cfg Config, reg *registry.Registry) (*Service, error) {
	...
	return &Service{
		...
		shard:                cfg.Shard,
		orderbooks:           make(map[int8]*orderbook.OrderBook),
...
func (s *Service) Start(ctx context.Context) error {
	s.nats.Subscribe(ctx, "orders."+s.shard+".>", "oms-"+s.shard, s.handleOrderEvent)
	s.nats.Subscribe(ctx, "p."+s.shard, "oms-price-"+s.shard, s.handlePriceTick)
	s.nats.Subscribe(ctx, "positions.>", "oms-"+s.shard+"-positions", s.handlePositionUpdate)
...
func (s *Service) handlePositionUpdate(data []byte) {
	...
	offset++
	if len(data) < offset+symbolLen+18 {
		return
	}
...
func (s *Service) publishOrderCancel(userID uint64, symbol string, orderID types.OrderID) {
	...
	s.nats.PublishBytes(context.Background(), "orders.cancel", buf)
}

func (s *Service) publishOrderTrim(userID uint64, symbol string, orderID types.OrderID, newQty int64) {
	...
	s.nats.PublishBytes(context.Background(), "orders.trim", buf)
}

func (s *Service) adjustReduceOnlyOrders(userID uint64, symbol string, maxSize int64) {
	userOrders := s.orders[userID]
	if userOrders == nil {
		return
	}
	...
	for _, order := range userOrders {
		...
		orderRemaining := int64(order.Quantity - order.Filled)
		if orderRemaining <= 0 {
			continue
		}
...
func (s *Service) getPositionSize(userID uint64, symbol string) int64 {
	...
	reply, err := s.nats.RequestReply(context.Background(), "portfolio.getPosition", buf, 5*time.Second)
	if err != nil {
		return 0
	}
	if len(reply) < 9 {
...
func (s *Service) placeNormalOrder(ctx context.Context, order *types.Order, input *OrderInput) (*OrderResult, error) {
	...
	if status == constants.ORDER_STATUS_FILLED || status == constants.ORDER_STATUS_CANCELED || status == constants.ORDER_STATUS_PARTIALLY_FILLED_CANCELED {
		s.CancelOrder(ctx, order.UserID, id.OrderID(order.ID))
	}
...
func (s *Service) publishOrderFilled(ctx context.Context, order *types.Order, matches []types.Match) {
	trades := make([]events.Trade, len(matches))
	for i, m := range matches {
		trades[i] = events.Trade{
			Symbol:       m.Trade.Symbol,
			TakerOrderID: id.OrderID(m.Trade.TakerOrderID),
			MakerOrderID: id.OrderID(m.Trade.MakerOrderID),
			TakerID:      m.Trade.TakerID,
			MakerID:      m.Trade.MakerID,
			Price:        int64(m.Trade.Price),
			Qty:          int64(m.Trade.Quantity),
		}
	}
	subject := "orders." + order.Symbol + ".FILLED"
	s.nats.Publish(ctx, subject, &events.OrderFilled{
		OrderID: id.OrderID(order.ID),
		Trades:  trades,
		Filled:  int64(order.Filled),
		Status:  order.Status,
...
func (s *Service) publishTrade(trade *types.Trade) {
	buf := make([]byte, 0, 10+len(trade.Symbol)+60)
	buf = append(buf, 0x01)
	buf = append(buf, byte(len(trade.Symbol)))
	buf = append(buf, trade.Symbol...)
	buf = binary.LittleEndian.AppendUint64(buf, trade.TakerID)
	buf = binary.LittleEndian.AppendUint64(buf, trade.MakerID)
	buf = binary.LittleEndian.AppendUint64(buf, uint64(trade.TakerOrderID))
	buf = binary.LittleEndian.AppendUint64(buf, uint64(trade.MakerOrderID))
	buf = binary.LittleEndian.AppendUint64(buf, uint64(trade.Price))
	buf = binary.LittleEndian.AppendUint64(buf, uint64(trade.Quantity))
	buf = append(buf, byte(trade.Category))
	buf = append(buf, byte(trade.TakerLeverage))
	buf = append(buf, byte(trade.MakerLeverage))

	s.nats.PublishBytes(context.Background(), "clearing.trade", buf)
}

func (s *Service) publishReserveRequest(ctx context.Context, userID uint64, symbol string, category, side int8, qty types.Quantity, price types.Price, leverage int8) error {
	...
	reply, err := s.nats.RequestReply(ctx, "clearing.reserve", buf, 5*time.Second)
	if err != nil {
		return err
	}
...
func (s *Service) publishReleaseRequest(ctx context.Context, userID uint64, symbol string, category, side int8, qty types.Quantity, price types.Price, leverage int8) {
	buf := make([]byte, 0, 10+len(symbol)+8+1+1+8+8+1)
	buf = append(buf, 0x02)
	buf = binary.LittleEndian.AppendUint64(buf, userID)
	buf = append(buf, byte(len(symbol)))
	...
	s.nats.PublishBytes(ctx, "clearing.release", buf)
}

func (s *Service) CancelOrder(ctx context.Context, userID uint64, orderID id.OrderID) error {
	userOrders := s.orders[userID]
	if userOrders == nil {
		return nil
	}

	order := userOrders[types.OrderID(orderID)]
	if order == nil {
		return nil
	}

	if order.Status == constants.ORDER_STATUS_UNTRIGGERED {
		s.triggerMonitor.Remove(types.OrderID(orderID))
	}
	...
	delete(userOrders, types.OrderID(orderID))
	if len(userOrders) == 0 {
	...
	subject := "orders." + order.Symbol + ".CANCELLED"
	s.nats.Publish(ctx, subject, &events.OrderCancelled{
		OrderID: orderID,
		Reason:  "user",
	})
...
func (s *Service) GetOrder(userID uint64, orderID id.OrderID) *types.Order {
	if userOrders := s.orders[userID]; userOrders != nil {
		return userOrders[types.OrderID(orderID)]
	}
	return nil
}
